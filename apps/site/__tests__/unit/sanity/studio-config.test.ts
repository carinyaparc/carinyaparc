...
describe('Sanity Studio Configuration', () => {
  ...
  describe('Plugin Registration', () => {
    ...
    it('registers structure tool plugin', async () => {
      const config = (await import('@/../sanity.config')).default;

      if (isSingleWorkspace(config)) {
        const hasStructureTool = config.plugins?.some(
          (plugin: unknown) =>
            plugin && typeof plugin === 'object' && 'name' in plugin && plugin.name === 'structure',
        );

        expect(hasStructureTool).toBe(true);
      }
    });

    it('registers vision tool plugin', async () => {
      const config = (await import('@/../sanity.config')).default;

      if (isSingleWorkspace(config)) {
        const hasVisionTool = config.plugins?.some(
          (plugin: unknown) =>
            plugin &&
            typeof plugin === 'object' &&
            'name' in plugin &&
            plugin.name === '@sanity/vision',
        );

        expect(hasVisionTool).toBe(true);
      }
    });

    it('registers presentation tool plugin', async () => {
      const config = (await import('@/../sanity.config')).default;

      if (isSingleWorkspace(config)) {
        const hasPresentationTool = config.plugins?.some(
          (plugin: unknown) =>
            plugin &&
            typeof plugin === 'object' &&
            'name' in plugin &&
        ...
        expect(hasPresentationTool).toBe(true);
      }
    });
  });
  ...
  describe('Presentation Tool Configuration', () => {
    it('configures preview URL with site origin', async () => {
      const config = (await import('@/../sanity.config')).default;

      if (isSingleWorkspace(config)) {
        const presentationPlugin = config.plugins?.find(
          (plugin: unknown) =>
            plugin &&
            typeof plugin === 'object' &&
            'name' in plugin &&
        ...
        // Type assertion for plugin with options
        const pluginWithOptions = presentationPlugin as {
          options?: { previewUrl?: { origin?: string } };
        };
        expect(pluginWithOptions.options?.previewUrl?.origin).toBe('http://localhost:3000');
      }
    });

    it('configures preview mode API route', async () => {
      const config = (await import('@/../sanity.config')).default;
      ...
      if (isSingleWorkspace(config)) {
        ...
        const pluginWithOptions = presentationPlugin as {
          options?: { previewUrl?: { previewMode?: { enable?: string } } };
        };

        expect(pluginWithOptions.options?.previewUrl?.previewMode?.enable).toBe('/api/draft');
      }
    });
  });
  ...
  describe('Vision Tool Configuration', () => {
    it('configures vision tool with correct API version', async () => {
      const config = (await import('@/../sanity.config')).default;

      if (isSingleWorkspace(config)) {
        const visionPlugin = config.plugins?.find(
          (plugin: unknown) =>
            plugin &&
            typeof plugin === 'object' &&
            'name' in plugin &&
        ...
        const pluginWithOptions = visionPlugin as { options?: { defaultApiVersion?: string } };
        expect(pluginWithOptions.options?.defaultApiVersion).toBe('2025-01-01');
      }
    });

    it('configures vision tool with correct dataset', async () => {
      const config = (await import('@/../sanity.config')).default;
      ...
      if (isSingleWorkspace(config)) {
        ...
        const pluginWithOptions = visionPlugin as { options?: { defaultDataset?: string } };
        expect(pluginWithOptions.options?.defaultDataset).toBe('test-dataset');
      }
    });
  });
});
